...
//  lunchify-ios
//
//  Created by Karim Karimov on 8/17/17.
...
: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate {
    ...
    open var tabsTextFont: UIFont = UIFont.systemFont(ofSize: 13, weight: UIFont.Weight.regular)
    ...
    func layoutSubViews() {
        ...
        frame.origin.x = 0.0
        frame.origin.y = self.tabsView!.frame.height
        frame.size.width = self.view.frame.width
...
//        self.tabsView!.setContentOffset(frame.origin, animated: true)
    ...
    func tabViewAtIndex(_ index: Int) -> TabView? {
        ...
        if (self.tabs[index] as UIView?) == nil {
            ...
            let tabView: TabView = TabView(frame: CGRect(x: 0.0, y: 0.0, width: tabViewContent.bounds.size.width, height: self.tabHeight))
